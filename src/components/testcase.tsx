...
it("should add phoneType to updatedFilters.phoneType if it's not already present", () => {
  const selectedAllFilters = [
    { attr: 'phoneType', title: 'Foldable' },
  ];
  ...
  selectedAllFilters.forEach((filter) => {
    switch (filter.attr) {
      ...
      case 'offerPrice': {
        const titleExists = updatedFilters.priceRange.some(
          (range) => range.title === filter.title
        );
  ...
  expect(updatedFilters.phoneType).toContain('Foldable');
});


it('should correctly set the 5G flag in updatedFilters', () => {
  const selectedAllFilters = [
    { attr: '5G', title: 'Yes' },
    { attr: '5G', title: 'No' },
  ];
  ...
  // Only apply the last one in sequence (simulate real switch override behavior)
  selectedAllFilters.forEach((filter) => {
    switch (filter.attr) {
      case '5G':
        updatedFilters['5G'] = filter.title?.toLowerCase() !== 'no';
  ...
  // "No" is the last one, so should be false
  expect(updatedFilters['5G']).toBe(false);
});

it('should skip unknown filter attr via default case', () => {
  const selectedAllFilters = [
    { attr: 'unknown', title: 'Some value' },
  ];
